...
$mutations = @(
  ...
  @{ n='test count drifted';      suite='form'; expect='22'; do={ (Get-Content "$Work\README.md" -Raw -Encoding UTF8) -creplace '24 form tests','25 form tests' | Set-Content "$Work\README.md" -Encoding UTF8 } }
  @{ n='invented pawn kind in a feature'; suite='form'; expect='23'; do={ $f = Join-Path $Work 'Tests\Pickle\Mod\Pickle\Features\02-the-gaze.feature'; (Get-Content $f -Raw -Encoding UTF8) -creplace '"Fingerspike"', '"Fleshbeast"' | Set-Content $f -Encoding UTF8 } }
  @{ n='docs name a ghost file';   suite='form'; expect='24'; do={ (Get-Content "$Work\README.md" -Raw -Encoding UTF8) -creplace 'TESTING.md','Platforms.xml' | Set-Content "$Work\README.md" -Encoding UTF8 } }
