...
workshop:     3806760893
remaining:
  - "verified: both Pickle passes are green. 5eb3 (English) and 1a55 (French) against 64f5de0 each report exitReason passed with 23 of 23, and 23 is the whole suite rather than a subset: the nine features declare 23 scenarios, none tagged @wip, none conditional. All four @review captures were opened and read - the settings window on its documented defaults in each language, and the inspect pane on the watcher reading 'Watching the contained entity.' and 'Observe l'entite captive.' What this does NOT yet earn is the stage: three of the checks TESTING.md's own conversion table owes have no scenario at all, listed separately below."
  ...
  - "unverified: the mod has never been loaded without Anomaly in a running game. TESTING.md's table asks scenario 1 for one pass per DLC state and only the DLC-on state has run. The out-of-game suites prove the class lives in Assembly-CSharp and the defs carry MayRequire, which is why the assembly loads at all; they cannot show what the game does with the DLC switched off. The launcher's -ThenWithout takes packageIds out of ModsConfig between launches under one lock, so this is a scenario to write rather than a missing capability. The shared DefInjected checker's two MayRequire translation-gating notices belong to this same DLC-off question."
