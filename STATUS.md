...
workshop:     3806760893
remaining:
  - "unverified: the nine Pickle features have run twice and never yet come back green. The second English pass 6612 played 23 of 23 against 6d297f6 with exitReason failed, 16 passed, and the seven failures again came from three faults in the suite rather than in the mod. One: the scenario named the patcher by packageId where Pickle records the display name, and was answered 'patched by Entity Gazing'. Two: the holder was spawned with no faction, and ListerBuildings.Add sorts a building into allBuildingsColonist on its faction once, at SpawnSetup - so JoyUtility.JoyKindsOnMapTempList, which walks only that list, never saw it, while the giver, which scans by def, did. Three: the watch range was measured as a straight line where WatchBuildingUtility lays a rect per cardinal direction, the range running along the facing axis and watchBuildingStandRectWidth sideways, which puts the far corner of 2~6 in a rect 5 wide at sqrt(40) = 6.32 cells. Every one of the three read out of Assembly-CSharp rather than guessed. All three fixed, and the @review capture of the job report fixed with them: it had been taken with nothing selected and the camera elsewhere, so it passed while showing no job report at all. Both passes resubmitted against 57097e7: English 20260925-113301-039-a657, French 20260925-113318-704-b7bf. The working tree must stay at 57097e7 until they report, since a request carries no SHA and staging copies whatever is on disk at launch."
...
updated:      2026-09-25, mod session
---
