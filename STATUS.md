...
workshop:     3806760893
remaining:
  - "unverified: one scenario of twenty-three is still red, in both languages, and it is the teardown hook rather than the mod. Passes a657 (English) and b7bf (French) against 57097e7 each played 23 with exitReason failed and 22 passed, both failing on 'an occupied platform is still a recreation source after a reload' - a scenario every one of whose own steps the Cucumber stream records as PASSED. Pickle invokes hooks by reflection, so the AfterScenario cleanup destroying a holder that belonged to the map the reload had already discarded surfaced as 'Exception has been thrown by the target of an invocation' with no hook named. Guarded on Find.Maps and wrapped, as was InterfaceSteps.Restore, which a new out-of-game test (36) found unguarded the first time it ran and which was the more dangerous of the two, running after every scenario in the suite. That test now checks the IL of every AfterScenario hook for a catch clause at offset 0, which is the only shape from which nothing can escape. A confirmation ticket on the reload feature alone is queued before the two final full passes."
